...
function encodeToRot13(str) {
    var alphabet = 'abcdefghijklmnopqrstuvwxyz', out = ''
    for (var i = 0; i < str.length; i++) {
        var temp = str[i].toLowerCase()
        if (temp == '?') out += temp
        else if (alphabet.search(temp.toLowerCase()) == -1) out += temp
        else if (alphabet.search(temp.toLowerCase()) + 13 > alphabet.length - 1) {
            out += (temp.toUpperCase() == str[i]) ? alphabet[alphabet.search(temp.toLowerCase()) + 13 - alphabet.length].toUpperCase() : alphabet[alphabet.search(temp.toLowerCase()) + 13 - alphabet.length]
        } else {
            out += (temp.toUpperCase() == str[i]) ? alphabet[alphabet.search(temp.toLowerCase()) + 13].toUpperCase() : alphabet[alphabet.search(temp.toLowerCase()) + 13]
        }
    }
    return out
}
